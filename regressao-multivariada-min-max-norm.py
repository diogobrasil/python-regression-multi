# regressao-multivariada-min-max-norm.py
"""
@file regressao-multivariada-min-max-norm.py
...
def main():
    ...
    os.makedirs("Figures/minmax/", exist_ok=True)
    ...
    X_norm, minimos, maximos = features_normalizes_by_min_max(X)
    ...
    print(f'Mínimo (min): {minimos}')
    print(f'Máximo (max): {maximos}')
    ...
    plt.grid(True)
    plt.savefig('Figures/minmax/minmax_convergencia_custo_multi.png', dpi=300, bbox_inches='tight')
    plt.savefig('Figures/minmax/minmax_convergencia_custo_multi.svg', format='svg', bbox_inches='tight')
    plt.show()
    ...
    example_norm = (example - minimos)/(maximos - minimos) # normaliza
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('Figures/minmax/minmax_convergencia_custo_vs_ne.png', dpi=300, bbox_inches='tight')
    plt.savefig('Figures/minmax/minmax_convergencia_custo_vs_ne.svg', format='svg', bbox_inches='tight')
    plt.show()
    ...
    theta_ne_norm[1:] = theta_ne[1:]/(maximos - minimos)
    theta_ne_norm[0] = theta_ne[0] - np.sum((minimos * theta_ne[1:])/(maximos - minimos))
    ...
    ax.legend()
    fig.savefig("Figures/minmax/minmax_superficie_GD_vs_NE.png", dpi=300)
    ...
    plt.savefig("Figures/minmax/minmax_contorno_GD_vs_NE.png", dpi=300)
    plt.show()
    ...
    theta_gd_orig[1:] = theta_gd[1:]*(maximos - minimos)  # coeficientes revertidos
    theta_gd_orig[0] = theta_gd[0] - np.sum(minimos * theta_gd[1:])  # intercepto
    ...
    fig2.tight_layout()
    fig2.savefig("Figures/minmax/minmax_ajuste_regressao_multivariada.png", dpi=300)
    plt.show()
